...
param([string]$dd = "\\1CServer\1CDistr", # путь до каталога с дистрибутивами платфоры 1С 8
      ...
      [string]$osa = "") # битность "32" или "64", если не задано, то равно битности ОС
...
" " >> $LogFile
"---------------------------------------------------------------------------------" >> $LogFile
"Версия скрипта: $ScriptVersion" >> $LogFile
"Параметры запуска скрипта: -dd '$dd' -dl '$dl' -dp '$dp' -ip '$ip' -iod '$iod' -osa='$osa'" >> $LogFile
